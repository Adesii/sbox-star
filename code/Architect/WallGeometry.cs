...
namespace Architect
{
	public class WallObject
	{
		...
		public void DebugDraw( Vector3 offset = new() )
		{
			...
			for ( int i = 0; i < HEMesh.Faces.Count; ++i )
			{
				...
				DebugOverlay.Text( $"room #{i} (height: {face.FloorHeight})", offset + new Vector3( face.Centroid, 1 ) * 32 );
				...
				foreach ( var halfEdge in face.HalfEdges )
				{
					...
					DebugOverlay.Line( offset + p1, offset + p2, Color.Green, 0, false );
				}
			}
			...
			for ( int i = 0; i < HEMesh.HalfEdges.Count; ++i )
			{
				...
				DebugOverlay.Line( offset + leftPoint, offset + rightPoint, h.Face.IsOuterFace ? Color.Orange : Color.Cyan, 0, false );
			}

			foreach ( var v in HEMesh.Vertices )
			{
				//DebugOverlay.Text( $"({v.X}, {v.Y})", new Vector3( v.X, v.Y, 0 ) * 32 );
				DebugOverlay.Sphere( offset + new Vector3( v.X, v.Y, 0 ) * 32, 5, v.Connections.Count == 0 ? Color.White.WithAlpha( 0.02f ) : Color.White.WithAlpha( 0.2f ), 0, false );
			}
		}
	}
}
